...
Soporte MULTIMARCA: Segundo octeto dinámico según la marca
"""
...
class PrintService3Attempts:
    ...
    def __init__(self, store_code: str):
        ...
        # ✅ OBTENER EL SEGUNDO OCTETO SEGÚN LA MARCA
    ...
    def _get_ips_from_idimpresora(self, id_impresora: str) -> list:
        ...
        ✅ AHORA USA EL SEGUNDO OCTETO DINÁMICO
        """
        if not id_impresora:
            return [f"10.{self.second_octet}.{self.store_number}.21"]  # Default con cadena dinámica
        ...
        if id_impresora_lower in self.ID_IMPRESORA_TO_OCTET:
            ...
            if isinstance(octets, list):
                ...
                return [f"10.{self.second_octet}.{self.store_number}.{octet}" for octet in octets]
            else:
                # Para caja1, caja2, linea: IP única
                return [f"10.{self.second_octet}.{self.store_number}.{octets}"]
        ...
        if match:
            num = match.group(1)
            if num == "1":
                return [f"10.{self.second_octet}.{self.store_number}.21"]  # caja1 = .21
            elif num == "2":
                return [f"10.{self.second_octet}.{self.store_number}.22"]  # caja2 = .22

        # Default a .21 (caja1/linea) con cadena dinámica
        return [f"10.{self.second_octet}.{self.store_number}.21"]
    ...
    async def _attempt_1_corregido(self, doc_type: str, document_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Obtener IPs basadas en idImpresora (✅ AHORA CON SEGUNDO OCTETO DINÁMICO)
    ...
    async def _attempt_3_emergency_corregido(self, doc_type: str, document_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Obtener IPs basadas en idImpresora deducido (✅ AHORA CON SEGUNDO OCTETO DINÁMICO)
            ips_to_try = self._get_ips_from_idimpresora(id_impresora)

            # Para emergencia, probar también otras IPs comunes (✅ CON SEGUNDO OCTETO DINÁMICO)
            all_ips = list(set(ips_to_try + [
                f"10.{self.second_octet}.{self.store_number}.21",  # caja1/linea
                f"10.{self.second_octet}.{self.store_number}.22",  # caja2
                f"10.{self.second_octet}.{self.store_number}.15",  # domi opción 1
                f"10.{self.second_octet}.{self.store_number}.78",  # domi opción 2
                f"10.{self.second_octet}.{self.store_number}.2",   # domi opción 3
            ]))
